...
src/
  ...
  pages/lp/
    professionelle-website-basel.astro  # Google Ads landing page — self-contained, no Nav component
functions/
...
Dedicated Google Ads conversion page. Self-contained single file — no `Nav.astro` or `Footer.astro`, uses `Layout.astro` for GTM/fonts/consent. Two independent form instances (hero + closing CTA), both POST to `/contact` and push `form_submit_success` to `dataLayer`.

**Sections (in order):** sticky LP header → hero (H1 + bullets + form) → Für wen → Was inklusive (2-col benefits/features) → Pakete (3 cards) → 3-step Prozess → Referenzen → Über Stefan → FAQ accordion → Abschluss-CTA → minimal footer.

**Placeholders to replace:**
- `photo-placeholder` div in "Über Stefan" → swap for `<img>` of Stefan
- `ref-img-placeholder` divs in Referenzen → swap for real screenshots

**Note:** `/lp/professionelle-website-basel` is currently picked up by `@astrojs/sitemap`. Add it to the sitemap exclusion list in `astro.config.mjs` if you don't want it indexed (e.g. to avoid duplicate content with the homepage).
...
6. **LP `/lp/professionelle-website-basel` — photo** — replace `photo-placeholder` div in "Über Stefan" with a real `<img>` of Stefan
7. **LP `/lp/professionelle-website-basel` — referenzen** — replace the 3 `ref-img-placeholder` divs with real project screenshots once available
8. **LP `/lp/professionelle-website-basel` — sitemap** — decide whether to exclude `/lp/professionelle-website-basel` from sitemap (add to `filter` in `astro.config.mjs`) to avoid duplicate-content issues with the homepage
